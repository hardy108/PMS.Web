...
namespace PMS.Web.Services.Controllers.Inventory
{
    ...
    {
        private const string _CONTROLLER_NAME = "GoodIssue"; //Replace with your own route name
        private GoodIssue _service;
        private AuthenticationServiceEstate _authenticationService;

        public GoodIssueController(AuthenticationServiceEstate authenticationService, PMSContextEstate context, AuditContext auditContext)
        {
            _authenticationService = authenticationService;
            _service = new GoodIssue(context, _authenticationService, auditContext);
        }

        [HttpGet]
        [Route("api/" + _CONTROLLER_NAME + "/list")]
        public IActionResult GetList([FromQuery]FilterGoodIssue param)
        {
        ...
        public IActionResult GeListCount([FromQuery]FilterGoodIssue filter)
        {
        ...
        public IActionResult GetListForSelect([FromQuery]FilterGoodIssue param)
        {
        ...
        {
            var webSession = _authenticationService.LoginBySession(Request.Headers);
            return Ok(_service.Approve(data,false, webSession.DecodedToken.UserName));
        }
