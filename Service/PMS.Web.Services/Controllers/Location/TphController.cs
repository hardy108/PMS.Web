...
namespace PMS.Web.Services.Controllers.Location
{
    ...
    {
        private const string _CONTROLLER_NAME = "Tph";
        private Tph _service;
        AuthenticationServiceEstate _authenticationService;

        public TphController(AuthenticationServiceEstate authenticationService, PMSContextEstate context,AuditContext auditContext)
        {
            _authenticationService = authenticationService;
            _service = new Tph(context, _authenticationService, auditContext);
        }
        ...
        public IActionResult GetListForSelect([FromQuery]FilterBlock param)
        {
            return Ok(_service.GetList(param).Select(c => new SelectOptionItem() { Id = c.TPHID, Text = c.CODE }));
        }
