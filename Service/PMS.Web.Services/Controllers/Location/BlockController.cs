...
{    
    ...
    {
        private const string _CONTROLLER_NAME = "Block"; //Replace with your own route name
        private Block _service;
        
        private AuthenticationServiceEstate _authenticationService;

        public BlockController(AuthenticationServiceEstate authenticationService, PMSContextEstate context, AuditContext auditContext)
        {
            _authenticationService = authenticationService;
            _service = new Block(context,authenticationService, auditContext);
        }
        ...
        [HttpGet]
        [Route("api/" + _CONTROLLER_NAME + "/listnurserybatch")]
        public IActionResult GetListNurseryBatch([FromQuery] string UnitID)
        {
            return Ok(_service.GetListNurseryBatch(UnitID));
        }
    }
}