...
{
    [ApiController]
    public class AttendanceGroupItemController : ControllerBase
    {
        private const string _CONTROLLER_NAME = "attendancegroupitem";
        private AttendanceGroupItem _service;
        AuthenticationServiceEstate _authenticationService;

        public AttendanceGroupItemController(AuthenticationServiceEstate authenticationService, PMSContextEstate context, AuditContext auditContext)
        {
            _authenticationService = authenticationService;
            _service = new AttendanceGroupItem(context, _authenticationService, auditContext);
        }
        ...
        {
            return Ok(_service.GetList(param).Select(c => new SelectOptionItem() { Id = c.ID, Text = c.ID + " - " + c.GROUPID}));
        }
