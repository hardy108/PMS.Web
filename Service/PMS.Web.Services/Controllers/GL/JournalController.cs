...
namespace PMS.Web.Services.Controllers.GL
{
    ...
    {
        private const string _CONTROLLER_NAME = "Journal"; //Replace with your own route name
        private Journal _service;
        private AuthenticationServiceEstate _authenticationService;

        public JournalController(AuthenticationServiceEstate authenticationService, PMSContextEstate context, AuditContext auditContext)
        {
            _authenticationService = authenticationService;
            _service = new Journal(context, _authenticationService, auditContext);
        }
        ...
        public IActionResult GeListCount([FromQuery]FilterJournal filter)
        {
        ...
        {
            return Ok(_service.GetList(param).Select(c => new SelectOptionItem() { Id = c.CODE, Text = c.CODE + " - " + c.NO + " - " + c.NOTE }));
        }
