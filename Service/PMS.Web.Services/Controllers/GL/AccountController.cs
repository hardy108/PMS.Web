...
namespace PMS.Web.Services.Controllers.GL
{
    ...
    {
        private const string _CONTROLLER_NAME = "Account"; //Replace with your own route name
        private Account _service;
        private AuthenticationServiceEstate _authenticationService;

        public AccountController(AuthenticationServiceEstate authenticationService, PMSContextEstate context,AuditContext auditContext)
        {
            _authenticationService = authenticationService;
            _service = new Account(context, _authenticationService,auditContext);
        }
        ...
        public IActionResult GetList([FromQuery]FilterAccount param)
        {
        ...
        public IActionResult GeListCount([FromQuery]FilterAccount filter)
        {
        ...
        public IActionResult GetListForSelect([FromQuery]FilterAccount param)
        {
        ...
        [Authorize]
        [HttpGet]
        [Route("api/" + _CONTROLLER_NAME + "/closeyear")]
        [Consumes("application/x-www-form-urlencoded")]
        public IActionResult CloseYear([FromQuery]string unitCode, int year, string accountCode, string userName)
        {
            var webSession = _authenticationService.LoginBySession(Request.Headers);
            return Ok(_service.CloseYear(unitCode,year,accountCode,webSession.DecodedToken.UserName));
        }
