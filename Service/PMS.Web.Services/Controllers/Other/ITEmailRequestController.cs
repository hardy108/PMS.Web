...
namespace PMS.Web.Services.Controllers.Organization
{
    ...
    [ApiController]
    public class ITEmailRequestController : ControllerBase
    {
        private const string _CONTROLLER_NAME = "itremail"; //Replace with your own route name
        

        private AuthenticationServiceEstate _authenticationService;
        private ITEmailRequest _service;
        
        public ITEmailRequestController(AuthenticationServiceEstate authenticationService,AuthenticationServiceHO authenticationServiceHO, PMSContextHO context,WFContext wfContext, IBackgroundTaskQueue taskQueue,IEmailSender emailSender, AuditContext auditContext)
        {
            _authenticationService = authenticationService;
            _service = new ITEmailRequest(context,wfContext, authenticationServiceHO, taskQueue,emailSender,auditContext);
        }
        ...
        {
            var webSession = _authenticationService.LoginBySession(Request.Headers);
            var record = _service.NewRecord(webSession.DecodedToken.UserName);
            record.CREATEDBY = webSession.DecodedToken.UserName;
            return Ok(record);        
        }
