...
{
    [ApiController]
    public class SalaryTypeMapController : ControllerBase
    {
        private const string _CONTROLLER_NAME = "SalaryTypeMap"; //Replace with your own route name
        private SalaryTypeMap _service;
        private AuthenticationServiceEstate _authenticationService;

        public SalaryTypeMapController(AuthenticationServiceEstate authenticationService, PMSContextEstate context, AuditContext auditContext)
        {
            _authenticationService = authenticationService;
            _service = new SalaryTypeMap(context, _authenticationService, auditContext);
        }

        [HttpGet]
        [Route("api/" + _CONTROLLER_NAME + "/list")]
        public IActionResult GetList([FromQuery]FilterSalaryTypeMap param)
        {
        ...
        public IActionResult GetListForSelect([FromQuery]FilterSalaryTypeMap param)
        {
        ...
        public IActionResult GetListCount([FromQuery]FilterSalaryTypeMap param)
        {
            var webSession = _authenticationService.LoginBySession(Request.Headers);            
