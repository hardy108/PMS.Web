...
namespace PMS.Web.Services.Controllers.Payroll
{
    ...
    {
        private const string _CONTROLLER_NAME = "SalaryType";
        private SalaryType _service;
        private AuthenticationServiceEstate _authenticationService;

        public SalaryTypeController(AuthenticationServiceEstate authenticationService, PMSContextEstate context,AuditContext auditcontext)
        {
            _authenticationService = authenticationService;
            _service = new SalaryType(context, _authenticationService,auditcontext);
        }
        ...
        {
            return Ok(_service.GetAll(param).Select(c => new SelectOptionItem() { Id = c.ID, Text = c.NAME }));            
        }
