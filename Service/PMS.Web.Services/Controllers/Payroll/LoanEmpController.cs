...
{
    [ApiController]
    public class LoanEmpController : ControllerBase
    {
        private const string _CONTROLLER_NAME = "LoanEmp"; //Replace with your own route name
        private Loan _service;
        private AuthenticationServiceEstate _authenticationService;

        public LoanEmpController(AuthenticationServiceEstate authenticationService, PMSContextEstate context, AuditContext auditContext)
        {
            _authenticationService = authenticationService;
            _service = new Loan(context, _authenticationService, auditContext);
        }
        ...
        {
            return Ok(_service.GetAll(param).Select(c => new SelectOptionItem() { Id = c.TRANID, Text = c.UNITID }));
        }
