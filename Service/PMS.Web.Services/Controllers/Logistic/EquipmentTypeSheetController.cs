...
{
    [ApiController]
    public class EquipmentTypeSheetController : ControllerBase
    {
        private const string _CONTROLLER_NAME = "BKU"; //Replace with your own route name
        private EquipmentTypeSheet _service;
        private AuthenticationServiceEstate _authenticationService;

        public EquipmentTypeSheetController(AuthenticationServiceEstate authenticationService, PMSContextEstate context, AuditContext auditContext)
        {
            _authenticationService = authenticationService;
            _service = new EquipmentTypeSheet(context, _authenticationService, auditContext);
        }
        ...
        {
            return Ok(_service.GetList(param).Select(c => new SelectOptionItem() { Id = c.ID, Text = c.EQUIPID }));
        }
