...
{
    [ApiController]
    public class RKHController : ControllerBase
    {
        private const string _CONTROLLER_NAME = "RKH"; //Replace with your own route name
        private RKH _service;
        private AuthenticationServiceEstate _authenticationService;

        public RKHController(AuthenticationServiceEstate authenticationService, PMSContextEstate context, AuditContext auditContext)
        {
            _authenticationService = authenticationService;
            _service = new RKH(context, _authenticationService, auditContext);
        }
        ...
        public IActionResult GetListCount([FromQuery]FilterAccount param)
        {
            var webSession = _authenticationService.LoginBySession(Request.Headers);
            param.UserName = webSession.DecodedToken.UserName;
        ...
        [HttpGet]
        [Route("api/" + _CONTROLLER_NAME + "/actual")]
        public IActionResult GetActuals([FromQuery]FilterRKH param)
        {
            return Ok(_service.GetActual(param));
        }
