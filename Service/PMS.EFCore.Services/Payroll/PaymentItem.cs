...
{
    public class PaymentItem : EntityFactory<TPAYMENTITEM, TPAYMENTITEM, FilterSalary, PMSContextBase>
    {
        private Period _servicePeriod;
        private Divisi _serviceDivisiName;
        private Employee _serviceEmp;
        private AuthenticationServiceBase _authenticationService;
        public PaymentItem(PMSContextBase context, AuthenticationServiceBase authenticationService, AuditContext auditContext) : base(context, auditContext)
        {
            _serviceName = "PaymentItem";
        ...
        protected override TPAYMENTITEM GetSingleFromDB(params  object[] keyValues)
        {
            string id = keyValues[0].ToString();
            return _context.TPAYMENTITEM.Include(b => b.DOCNONavigation).Include(a => a.EMP).SingleOrDefault(d => d.DOCNO.Equals(id));
        }

        public override IEnumerable<TPAYMENTITEM> GetList(FilterSalary filter)
        {
            var criteria = PredicateBuilder.True<TPAYMENTITEM>();
            try
            {
                ...
                if (filter.PageSize <= 0)
                    return _context.TPAYMENTITEM.Where(criteria).ToList();
                return _context.TPAYMENTITEM.Where(criteria).GetPaged(filter.PageNo, filter.PageSize).Results;
            }
            catch { return null; }
        }
    }
